...
export default function ReservationsOffre() {
            ...
            <div className="flex items-center gap-2 sm:gap-3 mb-3 sm:mb-4">
              ...
              <div>
                <h1 className="text-xl sm:text-2xl lg:text-3xl font-bold bg-gradient-to-r from-violet-600 via-blue-600 to-violet-700 bg-clip-text text-transparent">
                  La Liste Des Commandes Par Offres
                </h1>
                <p className="text-gray-600 text-xs sm:text-sm">Gérez les commandes pour chaque offre</p>
              </div>
            </div>
          </div>
            ...
            <div className="p-3 sm:p-4">
              ...
              {loading ? (
                ...
                <div className="text-center py-8 sm:py-12">
                  ...
                  <h3 className="text-lg sm:text-xl font-semibold text-gray-600 mb-2 sm:mb-3">Aucune offre trouvée</h3>
                  <p className="text-gray-500 text-sm sm:text-base">Aucune offre ne correspond à votre recherche.</p>
                </div>
              ) : (
                                      ...
                                      <tbody className="bg-white divide-y divide-gray-100">
                                        {reservations[offre.id].length > 0 ? (
                                          reservations[offre.id].map((res) => (
                                            <tr
                                              key={res.id}
                                              className="hover:bg-violet-50/30 transition-colors duration-200"
                                            >
                                              <td className="px-3 py-2 sm:px-4 sm:py-3 text-xs sm:text-sm font-medium text-gray-900">
                                                {res.user?.name || "Données utilisateur indisponibles"}
                                              </td>
                                              <td className="px-3 py-2 sm:px-4 sm:py-3 text-xs sm:text-sm text-gray-600">
                                                {res.user?.email || "Données utilisateur indisponibles"}
                                              </td>
                                              <td className="px-3 py-2 sm:px-4 sm:py-3 text-xs sm:text-sm text-gray-600">
                                                {res.user?.tel || "Données utilisateur indisponibles"}
                                              </td>
                                              <td className="px-3 py-2 sm:px-4 sm:py-3 text-xs sm:text-sm text-gray-600">
                                                {res.created_at
                                          ...
                                          <tr>
                                            <td
                                              colSpan={10}
                                              className="px-3 py-4 sm:px-4 sm:py-6 text-center text-gray-600 bg-gray-50/50"
                                            >
                                              <div className="flex flex-col items-center gap-2 sm:gap-3">
                                                <Package className="w-6 h-6 sm:w-8 h-8 text-gray-400" />
                                                <span className="font-medium text-xs sm:text-sm">Aucune commande pour cette offre.</span>
                                              </div>
                                            </td>
                                          </tr>
                                        )}
                    ...
                    <button
                      className={`px-3 py-1.5 sm:px-4 sm:py-2 rounded-md sm:rounded-lg border transition-all duration-200 ${
                        currentPage === 1
                          ...
                          : "text-gray-600 hover:bg-violet-50 hover:border-violet-300 border-gray-300"
                      }`}
                    ...
                    {Array.from({ length: totalPages }, (_, i) => (
                      <button
                        ...
                        className={`px-3 py-1.5 sm:px-4 sm:py-2 rounded-md sm:rounded-lg border transition-all duration-200 text-xs sm:text-sm ${
                          currentPage === i + 1
                            ? "bg-gradient-to-r from-violet-500 to-blue-600 text-white border-transparent shadow-sm"
                            : "text-gray-600 hover:bg-violet-50 hover:border-violet-300 border-gray-300"
                        }`}
