    ...
    <div className="flex h-screen bg-gray-100">
      ...
      <div className="w-64 bg-gray-800 text-white p-5 flex flex-col space-y-4">
        ...
        <nav className="space-y-2">
          <a href="/dashbord2" className="block p-2 hover:bg-gray-700 rounded">Dashboard</a>
      ...
      <div className="flex-1 container mx-auto px-4 py-8 relative">
        ...
        <div className="absolute top-4 right-4 flex items-center space-x-3 bg-white shadow-md p-3 rounded-lg">
          ...
          />
          <span className="text-gray-800 font-semibold">proprietaire</span>
        </div>
              ...
              {dashboardData.popular_services.map((service: any, index: number) => (
                <li key={index} className="flex flex-col md:flex-row justify-between items-start md:items-center">
                  <div className="md:w-2/3">
                    {/* <span className="text-lg font-semibold text-gray-700">{service.nom}</span>
                    <p className="text-gray-500">{service.description}</p>
                    <p className="text-gray-500">{service.adresse}</p> */}

                    {/* Détails du propriétaire */}
                    <div className="mt-4 p-4 bg-gray-50 rounded-md shadow-sm">
                      <h4 className="font-semibold text-gray-800">Propriétaire :</h4>
                      <p className="text-gray-700"><strong>Nom:</strong> {service.proprietaire.nom}</p>
                      <p className="text-gray-700"><strong>Adresse:</strong> {service.proprietaire.adresse}</p>
                      <p className="text-gray-700"><strong>Description:</strong> {service.proprietaire.description}</p>
                    </div> 
                  </div>
                  <span className="text-gray-500 mt-4 md:mt-0">{service.reservations_count} réservations</span>
                </li>
              ))}
            </ul>
          </div>
